...
type default{{.Type}} struct {
    {{if .Cache}}*mongoc.Model{{else}}*mongo.Model{{end}}
}

func New{{.Type}}(url, collection string{{if .Cache}}, c cachec.CacheConf{{end}}) {{.Type}} {
	return &default{{.Type}}{
...
func (m *default{{.Type}}) Insert(ctx context.Context, data *{{.Type}}) error {
    if !data.ID.Valid() {
...
func (m *default{{.Type}}) FindOne(ctx context.Context, id string) (*{{.Type}}, error) {
...
func (m *default{{.Type}}) Update(ctx context.Context, data *{{.Type}}) error {
    session, err := m.TakeSession()
    if err != nil {
        return err
    }
...
func (m *default{{.Type}}) Delete(ctx context.Context, id string) error {
    session, err := m.TakeSession()
    if err != nil {
        return err
    }
